...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    while change >= tolerance:
        ...
        for i in range(len(path)):
            p = (i-1)%len(path)
            n = (i+1)%len(path)
            newx = applysmooth(path[i][0], newpath[i][0], newpath[p][0], newpath[n][0], 
                weight_data, weight_smooth)
            newy = applysmooth(path[i][1], newpath[i][1], newpath[p][1], newpath[n][1], 
                weight_data, weight_smooth)
...
#printSmoothing(testpath2, smooth(testpath2))
# solution_check(smooth(testpath1), answer1)
