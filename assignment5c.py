...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        cte = 0.0
        if self.x < radius:
            # robot in left semi circle region centred at [radius, radius]
            cx, cy = radius, radius
            d = sqrt(((self.x - cx) ** 2) + ((self.y - cy) ** 2))
            cte = d - radius 
        elif self.x > 3 * radius:
            # robot in right semi circle region centered at [3*radius, radius]
            cx, cy = 3*radius, radius
            d = sqrt(((self.x - cx) ** 2) + ((self.y - cy) ** 2))
            cte = d - radius
        else:
            # robot in rectangular region
            if self.y >= 2 * radius:
                cte = self.y - 2 * radius
            elif self.y <= 0:
                cte = -self.y
            else:
                cte = -min(self.y, 2 * radius - self.y)
        return cte
