...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # Iterate over next N-1 measurements and movements
    omega = omega.expand(2*(N+num_landmarks), 2*(N+num_landmarks), [0, 1], [0, 1])
    xi = xi.expand(2*(N+num_landmarks), 1, [0, 1], [0])
    ...
    for i in range(len(data)):
        ...
        for k in range(len(Z)):
            ...
            # Update 8 cells in omega and 4 cells in xi
            update(omega, xi, mx, 2*i, 2*(N+l), measurement_noise)
            update(omega, xi, my, 2*i+1, 2*(N+l)+1, measurement_noise)             
        # Update for motion
        update(omega, xi, dx, 2*i, 2*(i+1), motion_noise)
        update(omega, xi, dy, 2*i+1, 2*(i+1)+1, motion_noise)        
