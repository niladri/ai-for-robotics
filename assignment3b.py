...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # Turning angle = (distance_moved / length_of_robot) * tan(steering_angle)
        ...
        if (abs(beta) < 0.001):
            ...
            # Turning radius = distance_moved / turning_angle
            R = motion[1] / beta
            # Measure center of turning
            cx = self.x - sin(self.orientation) * R
            cy = self.y + cos(self.orientation) * R
            # Calculate updated robot motion parameters
            X = cx + sin(self.orientation + beta) * R
            Y = cy - cos(self.orientation + beta) * R
            theta = (self.orientation + beta) % (2 * pi)
        
        # Simulate motion noise
        X = random.gauss(X, self.distance_noise)
        Y = random.gauss(Y, self.distance_noise)
        theta = random.gauss(theta, self.steering_noise) % (2 * pi)
    ...
    def sense(self, enable_noise = 0): #do not change the name of this function
        Z = []
