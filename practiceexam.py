...
delta = [[-1, 0], # go up
         ...
         [-1, 1], 
         [-1,-1]] 
stepCost = [1.0, 1.0, 1.0, 1.0, 1.5, 1.5, 1.5, 1.5]
...
def plan(warehouse, dropzone, todo):
    ...
    for i in range(len(todo)):
        goal = findGoal(warehouse, todo[i])
        warehouse[goal[0]][goal[1]] = 0
        sol = search(warehouse, dropzone, goal)
        cost += 2 * sol[0]
    return cost
