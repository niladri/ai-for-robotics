...
def update(omega, xi, m, p, l, noise):
    # i is postion index 
    # l is landmark index
    # m is measurement
    # Update 4 cells in omega and 2 cells in xi
    omega.value[p][p] += 1.0 / noise
    omega.value[p][l] += -1.0 / noise
    omega.value[l][p] += -1.0 / noise
    omega.value[l][l] += 1.0 / noise
    xi.value[p][0] += -m / noise
    xi.value[l][0] += m / noise
    
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    if not len(data) == N-1:
        raise(ValueError, "Data size does not match time steps")
    # Add the initial position to omega and xi
    omega = matrix([[1.0, 0.0], [0.0, 1.0]])
    xi = matrix([[world_size/2.0], [world_size/2.0]])
    # Expand for num_landmarks and one position
    omega = omega.expand(2*(1+num_landmarks), 2*(1+num_landmarks), [0, 1], [0, 1])
    xi = xi.expand(2*(1+num_landmarks), 1, [0, 1], [0])
    
    for i in range(len(data)):
        Z = data[i][0] # output of sense, list of landmark measurements
        move = data[i][1]
        dx = move[0]
        dy = move[1]
        for k in range(len(Z)):
            [l, mx, my] = Z[k] # landmark index, x measurement, y measurement
            update(omega, xi, mx, 0, 2*(1+l), measurement_noise)
            update(omega, xi, my, 1, 2*(1+l)+1, measurement_noise)             
        # Update for motion - first expand and then shrink
        expansionList = [0, 1]
        for i in range(2*num_landmarks):
            expansionList.append(i+4)
        dims = omega.dimx
        omega = omega.expand(dims+2, dims+2, expansionList)
        xi = xi.expand(dims+2, 1, expansionList, [0])
        update(omega, xi, dx, 0, 2, motion_noise)
        update(omega, xi, dy, 1, 3, motion_noise)
        takeList = [i+2 for i in range(2*(1+num_landmarks))]
        A = omega.take([0, 1], takeList)
        B = omega.take([0, 1])
        C = xi.take([0, 1], [0])
        omega2 = omega.take(takeList)
        xi2 = xi.take(takeList, [0])
